import ActionButtons from "@/components/DashboardPage/DepartamentosPage/Educacional/Treinadores/ActionButtons";
import Cards from "@/components/DashboardPage/DepartamentosPage/Educacional/Treinadores/Cards";
import Memberlist from "@/components/DashboardPage/DepartamentosPage/Educacional/Treinadores/Memberlist";
...
export default async function EducacitionalPage() {
  ...
  const verifyMemberOfTreinadores = await verifyMember();
  const allUsers = await getAllUsers();
  const treinadoreClasses = await getClasses();
  ...
  if (
    ...
    if (currentUser) {
      if (verifyMemberOfTreinadores) {
        const containsUser = verifyMemberOfTreinadores[0].profiles.some(
          (profile: Profile) => {
      ...
      <>
        <div className="flex w-full gap-16">
          <div className="w-full flex flex-col gap-8 h-full">
            <Cards
              members={
                verifyMemberOfTreinadores[0]
                  ? verifyMemberOfTreinadores[0].profiles.length
                  : 0
              }
            />
          ...
          <div className="flex flex-col w-52 gap-8 h-full">
            ...
            <Memberlist
              members={verifyMemberOfTreinadores}
              roles={roles}
              userList={userList}
            />
          </div>
